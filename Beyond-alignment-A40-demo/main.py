...
class experiment:
    ...
    def prepare(self):
        ...
        if self.args.use_pseudo_expand_triple:
            self.args.save_path = self.args.save_path + '-pseudo_expand_triple' + str(self.args.pseudo_expand_triple_weight)
            self.args.save_path = self.args.save_path + '-replace' + str(self.args.pseudo_expand_replace_ratio)
        
        if self.args.use_triple_infonce:
            ...
            # 如果设置了动态增长，显示start->end，否则显示固定值
            if hasattr(self.args, 'pseudo_triple_selection_ratio_start') and self.args.pseudo_triple_selection_ratio_start is not None:
                self.args.save_path = self.args.save_path + '-select' + str(self.args.pseudo_triple_selection) + '-' + str(self.args.pseudo_triple_selection_ratio_start) + 'to' + str(self.args.pseudo_triple_selection_ratio) + '-warmup' + str(self.args.pseudo_triple_selection_ratio_warmup)
            else:
                self.args.save_path = self.args.save_path + '-select' + str(self.args.pseudo_triple_selection) + '-' + str(self.args.pseudo_triple_selection_ratio)
        ...
        if self.args.use_inter_triples:
            self.args.save_path = self.args.save_path + '-use_inter-graph_update_K' + str(self.args.graph_update_interval)
        ...
        if self.args.use_pseudo_expand_triple:
            self.args.log_path = self.args.log_path + '-pseudo_expand_triple' + str(self.args.pseudo_expand_triple_weight)
            self.args.log_path = self.args.log_path + '-replace' + str(self.args.pseudo_expand_replace_ratio)
        ...
        if self.args.use_triple_infonce:
            ...
            # 如果设置了动态增长，显示start->end，否则显示固定值
            if hasattr(self.args, 'pseudo_triple_selection_ratio_start') and self.args.pseudo_triple_selection_ratio_start is not None:
                self.args.log_path = self.args.log_path + '-select' + str(self.args.pseudo_triple_selection) + '-' + str(self.args.pseudo_triple_selection_ratio_start) + 'to' + str(self.args.pseudo_triple_selection_ratio) + '-warmup' + str(self.args.pseudo_triple_selection_ratio_warmup)
            else:
                self.args.log_path = self.args.log_path + '-select' + str(self.args.pseudo_triple_selection) + '-' + str(self.args.pseudo_triple_selection_ratio)
        ...
        # 添加图结构更新配置信息到日志路径
        if self.args.use_inter_triples:
            self.args.log_path = self.args.log_path + '-use_inter-graph_update_K' + str(self.args.graph_update_interval)
        
        # 添加 InfoNCE 损失配置信息到日志路径
        if getattr(self.args, 'use_triple_infonce', False):
            self.args.log_path = self.args.log_path + '-infonce' + str(self.args.triple_infonce_weight) + '-temp' + str(self.args.triple_infonce_temperature)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-margin', dest='margin', default=12.0, help='')
    parser.add_argument('-gpu', dest='gpu', default=0)

    parser.add_argument('-encoder', dest='encoder', default='lookup', help='lookup, lookup_attn,lookup_gat')
    ...
    parser.add_argument('-pseudo_align_threshold', dest='pseudo_align_threshold', default=0.05, type=float, help='伪对齐对的置信度阈值λ')
    
    '''Pseudo Entity Expand Triple Loss'''
    parser.add_argument('-use_pseudo_expand_triple', dest='use_pseudo_expand_triple', default='False', help='是否使用基于伪实体对扩展的三元组损失')
    parser.add_argument('-pseudo_expand_triple_weight', dest='pseudo_expand_triple_weight', default=0.1, type=float, help='伪实体扩展三元组损失的权重')
    parser.add_argument('-pseudo_expand_replace_ratio', dest='pseudo_expand_replace_ratio', default=1.0, type=float, help='伪实体替换的比例（0.0-1.0），控制部分实体替换')
    
    '''Triple InfoNCE Loss'''
    parser.add_argument('-use_triple_infonce', dest='use_triple_infonce', default='False', type=str, help='是否使用基于扩展三元组的 InfoNCE 损失 (True/False)')
    ...
    parser.add_argument('-triple_infonce_weight', dest='triple_infonce_weight', default=0.1, type=float, help='InfoNCE 损失的权重')
    ...
    parser.add_argument('-pseudo_triple_selection_ratio', dest='pseudo_triple_selection_ratio', default=1.0, type=float, 
                        help='三元组选择比例k%（0.0-1.0），选择前k%的三元组或伪实体（如果设置了start和end，则此参数作为最终比例）')
    parser.add_argument('-pseudo_triple_selection_ratio_start', dest='pseudo_triple_selection_ratio_start', default=0.25, type=float, 
                        help='三元组选择比例的初始值（0.0-1.0），如果设置则启用动态增长')
    parser.add_argument('-pseudo_triple_selection_ratio_warmup', dest='pseudo_triple_selection_ratio_warmup', default=50, type=int, 
                        help='三元组选择比例从初始值增长到最终值需要的epoch数（默认50）')
    ...
    parser.add_argument('-graph_update_interval', dest='graph_update_interval', default=5, type=int, 
                        help='图结构更新间隔（每隔K轮更新一次图间三元组结构）')
    parser.add_argument('-use_inter_triples', dest='use_inter_triples', default='False', type=str,
