...
class TripleInfoNCELoss(nn.Module):
    ...
    - 正样本：h+r+t (完整三元组，仅扩展三元组)
    - 负样本：批次内负样本（同一批次中其他样本的 h+r+t）
    """
    ...
    def compute_positive_emb(self, h_emb, r_emb, t_emb):
        """
        计算正样本表示 h+r+t
        :param h_emb: [batch_size, emb_dim] 头实体嵌入
        :param r_emb: [batch_size, emb_dim] 关系嵌入
        :param t_emb: [batch_size, emb_dim] 尾实体嵌入
        :return: [batch_size, emb_dim] 正样本表示
        """
        # 对于 TransE: h+r+t
        positive_emb = h_emb + r_emb + t_emb
        return F.normalize(positive_emb, p=2, dim=1)
    
    def get_in_batch_negatives(self, positive_emb, batch_size):
        """
        获取批次内负样本
        :param positive_emb: [batch_size, emb_dim] 正样本嵌入
        :param batch_size: 批次大小
        :return: [batch_size, batch_size-1, emb_dim] 批次内负样本嵌入
        """
        # 对于每个样本，同一批次中其他样本的正样本就是负样本
        # positive_emb: [batch_size, emb_dim]
        # 扩展为 [batch_size, batch_size, emb_dim]
        in_batch_negs = positive_emb.unsqueeze(0).expand(batch_size, -1, -1)
        
        # 移除对角线（正样本自己）
        mask = ~torch.eye(batch_size, dtype=torch.bool, device=positive_emb.device)
    ...
    def forward(self, h_emb, r_emb, t_emb):
        ...
        positive_emb = self.compute_positive_emb(h_emb, r_emb, t_emb)  # [batch_size, emb_dim]
        
        # 计算正样本相似度
        pos_sim = (query_emb * positive_emb).sum(dim=1, keepdim=True) / self.temperature  # [batch_size, 1]
        
        # 获取批次内负样本
        in_batch_negs = self.get_in_batch_negatives(positive_emb, batch_size)  # [batch_size, batch_size-1, emb_dim]
