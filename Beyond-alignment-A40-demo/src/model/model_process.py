...
class CrossLinkPredictionTrainBatchProcessor():
    ...
    def process_epoch(self, model, optimizer):
        ...
        if self.args.use_pseudo_alignment:
            ...
            # 如果使用伪扩展三元组，在第一个batch之前就添加伪扩展三元组
            if self.args.use_pseudo_expand_triple:
                replace_ratio = getattr(self.args, 'pseudo_expand_replace_ratio', 1.0)
                # 计算动态选择比例
                selection_ratio = self._compute_dynamic_selection_ratio()
                self.args.logger.info(f'[DEBUG] Epoch {current_epoch}: Starting add_facts_using_pseudo_entities (replace_ratio={replace_ratio}, selection_ratio={selection_ratio})')
                num_added = self.add_facts_using_pseudo_entities(model, replace_ratio, selection_ratio)
                self.args.logger.info(f'[DEBUG] Epoch {current_epoch}: Added {num_added} pseudo expand triples')
            else:
                self.args.logger.info(f'[DEBUG] Epoch {current_epoch}: use_pseudo_expand_triple is False, skipping add_facts_using_pseudo_entities')
        else:
            self.args.logger.info(f'[DEBUG] Epoch {current_epoch}: use_pseudo_alignment is False, skipping compute_pseudo_alignment_pairs')

        # 更新图间三元组结构（无论是否使用图间三元组，都计算统计信息用于日志输出）
        use_inter_triples = getattr(self.args, 'use_inter_triples', False)
        graph_update_interval = getattr(self.args, 'graph_update_interval', 5)
        
        # 用于保存图结构统计信息
        graph_stats = None
        
        # 无论是否使用图间三元组，都计算图结构统计信息（用于日志输出）
        # 如果使用图间三元组，则每隔 K 轮更新一次图结构；否则只在需要时计算统计信息
        should_update_graph = use_inter_triples and (current_epoch == 0 or (current_epoch % graph_update_interval == 0))
        should_compute_stats = not use_inter_triples or should_update_graph
        ...
        if should_compute_stats:
            ...
            if use_inter_triples and should_update_graph:
                self.args.logger.info(
                    f'Epoch {current_epoch}: Graph structure updated - '
                    f'Original: {graph_stats["num_original_triples"]}, '
                    f'Known Expand: {graph_stats["num_known_expand_triples"]}, '
                    f'Pseudo Expand: {graph_stats["num_pseudo_expand_triples"]}'
                )
            ...
            try:
                for idx_b, batch in enumerate(tqdm(range(2 * len(self.head_data_loader)))):
                    ...
                    if is_expand is not None and is_ea is not None:
                        ...
                        # 只统计正样本（label > 0）
                        # by的形状是[batch_size*(1+neg_ratio)]，其中正样本label=1，负样本label=-1
                        # is_expand和is_ea的形状也是[batch_size*(1+neg_ratio)]
                        # 需要确保is_expand和is_ea的长度与by匹配
                        by_flat = by.reshape(-1) if by.dim() > 1 else by
                        positive_mask = (by_flat > 0).bool()
                        
                        # 确保is_expand和is_ea的长度与positive_mask匹配
                        if len(is_expand) != len(positive_mask):
                            # 如果长度不匹配，调整is_expand和is_ea的长度
                            min_len = min(len(is_expand), len(positive_mask))
                            is_expand = is_expand[:min_len]
                            is_ea = is_ea[:min_len]
                            positive_mask = positive_mask[:min_len]
                        ...
                        if len(is_expand) == len(positive_mask):
                            ...
                            # 如果有伪扩展三元组，计算其损失（只对正样本计算）
                            if self.args.use_pseudo_expand_triple and pseudo_expand_mask.any():
                                # 确保pseudo_expand_mask的长度与subsampling_weight匹配
                                # subsampling_weight的形状也是[batch_size*(1+neg_ratio)]
                                if len(pseudo_expand_mask) == len(subsampling_weight):
                                    pseudo_expand_loss = self.compute_pseudo_expand_triple_loss(
                                        model, bh, br, bt, by, bc, subsampling_weight, pseudo_expand_mask, mode
                                    )
                                    # 确保pseudo_expand_loss是标量
                                    if isinstance(pseudo_expand_loss, torch.Tensor) and pseudo_expand_loss.dim() > 0:
                                        pseudo_expand_loss = pseudo_expand_loss.mean()
                                    batch_loss = batch_loss + self.args.pseudo_expand_triple_weight * pseudo_expand_loss
                            
                            # 计算基于扩展三元组的 InfoNCE 损失
                            if getattr(self.args, 'use_triple_infonce', False) and self.triple_infonce_loss_fn is not None:
                                # 只对扩展三元组（is_expand=True）计算 InfoNCE 损失
                                expand_mask = is_expand & positive_mask
                                if expand_mask.any():
                                    infonce_loss = self.compute_triple_infonce_loss(
                                        model, bh, br, bt, expand_mask, mode
                                    )
                                    if isinstance(infonce_loss, torch.Tensor) and infonce_loss.dim() > 0:
                                        infonce_loss = infonce_loss.mean()
                                    triple_infonce_weight = getattr(self.args, 'triple_infonce_weight', 0.1)
                                    batch_loss = batch_loss + triple_infonce_weight * infonce_loss
    ...
    def compute_pseudo_expand_triple_loss(self, model, bh, br, bt, by, bc, subsampling_weight, pseudo_expand_mask, mode):
        """
        计算伪扩展三元组的损失
        :param model: 模型
        :param bh, br, bt: batch中的subject, relation, object
        :param by: batch中的label
        :param bc: batch中的confidence
        :param subsampling_weight: 子采样权重
        :param pseudo_expand_mask: 伪扩展三元组的mask
        :param mode: 'head-batch' 或 'tail-batch'
        :return: 伪扩展三元组损失
        """
        if not pseudo_expand_mask.any():
            return torch.tensor(0.0, device=bh.device, requires_grad=True)
        
        # 提取伪扩展三元组
        pseudo_bh = bh[pseudo_expand_mask]
        pseudo_br = br[pseudo_expand_mask]
        pseudo_bt = bt[pseudo_expand_mask]
        pseudo_by = by[pseudo_expand_mask]
        pseudo_bc = bc[pseudo_expand_mask]
        pseudo_subsampling_weight = subsampling_weight[pseudo_expand_mask]
        
        # 计算伪扩展三元组的损失
        jobs = {
            'sub_emb': {'opt': 'ent_embedding', 'input': {"indexes": pseudo_bh}, 'mode': mode},
            'rel_emb': {'opt': 'rel_embedding', 'input': {"indexes": pseudo_br}, 'mode': mode},
            'obj_emb': {'opt': 'ent_embedding', 'input': {"indexes": pseudo_bt}, 'mode': mode},
        }
        pred = model.forward(jobs=jobs, stage='train', mode=mode, margin=self.args.margin)
        pseudo_expand_triple_loss = model.loss(pred, pseudo_by, pseudo_subsampling_weight, pseudo_bc, neg_ratio=self.args.neg_ratio)
        
        return pseudo_expand_triple_loss
    ...
    def _compute_dynamic_selection_ratio(self):
        """
        根据当前epoch动态计算三元组选择比例
        如果设置了start和end，则线性增长；否则使用固定比例
        :return: 当前epoch的选择比例
        """
        ratio_start = getattr(self.args, 'pseudo_triple_selection_ratio_start', None)
        ratio_end = getattr(self.args, 'pseudo_triple_selection_ratio', 1.0)
        warmup_epochs = getattr(self.args, 'pseudo_triple_selection_ratio_warmup', 50)
        current_epoch = getattr(self.args, 'epoch', 0)
        ...
        # 线性增长策略：从ratio_start线性增长到ratio_end
        if current_epoch >= warmup_epochs:
            # 已经达到warmup，使用最终比例
            return ratio_end
        else:
            # 线性插值
            progress = current_epoch / max(warmup_epochs, 1)
            current_ratio = ratio_start + (ratio_end - ratio_start) * progress
    ...
    def add_facts_using_pseudo_entities(self, model, replace_ratio=1.0, selection_ratio=None):
        ...
        :param selection_ratio: 三元组选择比例，如果为None则使用args中的值或动态计算
        :return: 添加的伪扩展三元组数量
        """
        if selection_ratio is None:
            selection_ratio = getattr(self.args, 'pseudo_triple_selection_ratio', 1.0)
        
        num_added_head = self.head_dataset.add_facts_using_pseudo_entities(model, replace_ratio, selection_ratio)
        num_added_tail = self.tail_dataset.add_facts_using_pseudo_entities(model, replace_ratio, selection_ratio)
