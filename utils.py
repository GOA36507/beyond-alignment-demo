...
def retype_parameters(args):
    ...
    # Graph Structure Update 参数类型转换
    if hasattr(args, 'use_inter_triples'):
        if isinstance(args.use_inter_triples, str):
            args.use_inter_triples = True if args.use_inter_triples == 'True' else False
        elif isinstance(args.use_inter_triples, bool):
            pass  # 已经是bool类型
        else:
            args.use_inter_triples = bool(args.use_inter_triples)
    # Triple InfoNCE Loss 参数类型转换
    if hasattr(args, 'use_triple_infonce'):
        if isinstance(args.use_triple_infonce, str):
            args.use_triple_infonce = True if args.use_triple_infonce == 'True' else False
        elif isinstance(args.use_triple_infonce, bool):
            pass  # 已经是bool类型
        else:
            args.use_triple_infonce = bool(args.use_triple_infonce)
    # Pseudo Triple Loss 参数类型转换
    if hasattr(args, 'use_pseudo_triple_loss'):
        if isinstance(args.use_pseudo_triple_loss, str):
            args.use_pseudo_triple_loss = True if args.use_pseudo_triple_loss == 'True' else False
        elif isinstance(args.use_pseudo_triple_loss, bool):
            pass  # 已经是bool类型
        else:
            args.use_pseudo_triple_loss = bool(args.use_pseudo_triple_loss)
    # Entropy Selection 参数类型转换
    if hasattr(args, 'use_entropy_selection'):
        if isinstance(args.use_entropy_selection, str):
            args.use_entropy_selection = True if args.use_entropy_selection == 'True' else False
        elif isinstance(args.use_entropy_selection, bool):
            pass  # 已经是bool类型
        else:
            args.use_entropy_selection = bool(args.use_entropy_selection)
...
def load_align_pair(path, file_name):
    ...
    return pairs
# def load_align_pair(path, file_name):
#     cross_path = os.path.join(path, "cross")
#     ds = load_dataset("text", data_files={"train": os.path.join(cross_path, file_name)})["train"]["text"]
#     pairs = []
#     for line in ds:
#         parts = line.split()
