...
class CrossLinkPredictionTrainDatasetMarginLoss(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 将subsampling_weight扩展到所有样本（正样本+负样本）
        subsampling_weight_expanded = subsampling_weight.repeat(num_samples)
        return fact, label, conf_expanded, self.mode, subsampling_weight_expanded, mf, torch.ones(len(fact), dtype=torch.bool)*is_expand, torch.ones(len(fact), dtype=torch.bool)*is_ea
    ...
    def _compute_triple_scores_batch(self, model, triples, batch_size=256):
        """
        批量计算三元组的得分（使用模型），提高效率
        :param model: 模型
        :param triples: 三元组列表 [(s, r, o, pseudo_entity, is_subject), ...]
        ...
        with torch.no_grad():
            for i in range(0, len(triples), batch_size):
                ...
                batch_scores = model.forward(jobs=jobs, stage='train', mode=mode, margin=self.args.margin)
    ...
    def add_facts_using_pseudo_entities(self, model, replace_ratio=1.0):
        """
        基于伪实体对构造扩展三元组，支持基于熵的top-p筛选策略
        :param model: 模型，用于计算三元组得分
        :param replace_ratio: 实体替换的比例（0.0-1.0），控制部分实体替换
        :return: 添加的伪扩展三元组数量
        """
        self.args.logger.info(
            f'[DEBUG] add_facts_using_pseudo_entities: Starting (replace_ratio={replace_ratio})'
        )
        ...
        if selection_strategy == 'random':
            ...
            # 按得分排序选择 - 使用批量计算加速
            self.args.logger.info(f'[DEBUG] Computing scores for {len(candidate_triples)} triples in batches...')
            ...
            self.args.logger.info(f'[DEBUG] Finished computing scores for {len(triple_scores)} triples')
            ...
            if use_entropy_selection:
                ...
                # 计算均值和方差（numpy 方式）
                mu = scores.mean()
                var = scores.var(ddof=0)  # 使用总体方差（ddof=0），与 torch.var 一致
                
                n_sigma = getattr(self.args, 'entropy_n_sigma', 2.0)
                entropy_threshold = getattr(self.args, 'entropy_threshold', 10.0)
                
                self.args.logger.info(
                    f'[DEBUG] Entropy selection: mu={mu:.4f}, var={var:.4f}, '
                    f'n_sigma={n_sigma}, threshold={entropy_threshold}'
                )
                ...
                for triple, score in triple_scores:
                    # 截断高斯分布概率
                    clamped_diff = min(0.0, score - mu)  # score > mu 时为 0
                    ...
                    if cumulative_entropy > entropy_threshold:
                        self.args.logger.info(
                            f'[DEBUG] Entropy selection: Stopped at {len(selected_triples)+1}/{len(triple_scores)} triples, '
                            f'cumulative_entropy={cumulative_entropy:.4f}'
                        )
                        break
        ...
        #     if use_entropy_selection:
        #         # 熵筛选模式下，选择所有
        #         selected_entity_set = set([item[0] for item in sorted_entities])
        #     else:
        #         selection_ratio = getattr(self.args, 'pseudo_triple_selection_ratio', 1.0)
        ...
        if len(facts) > 0:
            if use_entropy_selection:
                self.args.logger.info('Add {} pseudo expand triples using {} strategy with entropy selection!'.format(
                    len(facts), selection_strategy))
            else:
                selection_ratio = getattr(self.args, 'pseudo_triple_selection_ratio', 1.0)
...
class RelationBatchSampler(BatchSampler):
    ...
    def __iter__(self):
        # [RPG 已禁用] 简化为 use_attn=False 的分支
